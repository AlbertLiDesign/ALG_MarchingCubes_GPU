    ...
    public class MarchingCubes_GPU
    {
        ...
        public void runClassifyVoxel()
        {
            ...
            gpu.For(0, numVoxels, i =>
            {
                ...
                //计算cube中的8个点对应的value
                float d0 = ComputeValue(d_samplePts, d_voxelV[i * 8], d_samplePts.Length);
                float d1 = ComputeValue(d_samplePts, d_voxelV[i * 8 + 1], d_samplePts.Length);
                float d2 = ComputeValue(d_samplePts, d_voxelV[i * 8 + 2], d_samplePts.Length);
                float d3 = ComputeValue(d_samplePts, d_voxelV[i * 8 + 3], d_samplePts.Length);
                float d4 = ComputeValue(d_samplePts, d_voxelV[i * 8 + 4], d_samplePts.Length);
                float d5 = ComputeValue(d_samplePts, d_voxelV[i * 8 + 5], d_samplePts.Length);
                float d6 = ComputeValue(d_samplePts, d_voxelV[i * 8 + 6], d_samplePts.Length);
                float d7 = ComputeValue(d_samplePts, d_voxelV[i * 8 + 7], d_samplePts.Length);
        ...
        public List<Point3d> runExtractIsoSurfaceGPU()
        {
            ...
            {
                //Compute cubeValues of 8 vertices
                d_cubeValues[i * 8] = ComputeValue(d_samplePts, d_model_voxelActive[i * 8], d_samplePts.Length);
                d_cubeValues[i * 8 + 1] = ComputeValue(d_samplePts, d_model_voxelActive[i * 8 + 1], d_samplePts.Length);
                d_cubeValues[i * 8 + 2] = ComputeValue(d_samplePts, d_model_voxelActive[i * 8 + 2], d_samplePts.Length);
                d_cubeValues[i * 8 + 3] = ComputeValue(d_samplePts, d_model_voxelActive[i * 8 + 3], d_samplePts.Length);
                d_cubeValues[i * 8 + 4] = ComputeValue(d_samplePts, d_model_voxelActive[i * 8 + 4], d_samplePts.Length);
                d_cubeValues[i * 8 + 5] = ComputeValue(d_samplePts, d_model_voxelActive[i * 8 + 5], d_samplePts.Length);
                d_cubeValues[i * 8 + 6] = ComputeValue(d_samplePts, d_model_voxelActive[i * 8 + 6], d_samplePts.Length);
                d_cubeValues[i * 8 + 7] = ComputeValue(d_samplePts, d_model_voxelActive[i * 8 + 7], d_samplePts.Length);
